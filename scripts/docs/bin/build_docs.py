...
#    print("Looking for: "+sectionname)
    ...
    if "" in section and sectionname not in ["Notes:", "Examples:"]:
        ...
        message = f'{item["signature"]} has a blank line in {sectionname}'
        warn(message)
        LINTS.append({
            "file": item["file"],
    ...
    for chunk in docstrings:
        if not chunk[2].startswith("==="):
            ...
            if modulename not in docs and os.environ.get("GITHUB_ACTIONS", default=None):
                print(
                    f"::error file={CHUNK_FILE},line={CHUNK_LINE},title='Unknown module'::Found a reference to module '{modulename}', but that module has no definition anywhere"
                )
    ...
    for itemname in raw_module["items"]:
        ...
        for section in ["Parameters", "Returns", "Notes", "Examples"]:
            if f'{section}:' in chunk:
                item[section.lower()] = get_section_from_chunk(chunk, f'{section}:', item)
        ...
        try:
            if item['desc'].startswith("Alias for [`"):
                ...
                if len(item["returns"]) == 0 and not ARGUMENTS.standalone:
                    message = f"RETURN COUNT ERROR: '{sig_without_return}' does not specify a return value"
                    warn(message)
                    LINTS.append({
                        "file": item["file"],
                        "line": int(item["lineno"]),
